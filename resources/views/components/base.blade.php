  ...
  <body>
      ...
      <div class="container d-flex flex-column flex-md-row align-items-center p-3">
        ...
        <div class="flex-row my-3 my-md-0">
          ...
          <a href="/profile/{{auth()->user()->username}}" class="mr-2">
            <img title="My Profile" data-toggle="tooltip" 
            data-placement="bottom" style="width: 32px; height: 32px; border-radius: 16px" 
            src="{{auth()->user()->avatar}}" />
          </a>
          <a class="btn btn-sm btn-success mr-2" href="/create-new-post">Create Post</a>
    ...
    @if (session()->has('success_msg'))
      <div class="container container--narrow">
        <div class="alert alert-success text-center">{{session("success_msg")}}
      </div>
    </div>
    @endif
    @if (session()->has('error_msg'))
    <div class="container container--narrow">
      <div class="alert alert-danger text-center">{{session("error_msg")}}
    </div>
  </div>
    @endif
{{$slot}}
